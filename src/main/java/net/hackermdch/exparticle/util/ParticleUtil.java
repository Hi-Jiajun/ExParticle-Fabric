...
public class ParticleUtil {
    ...
    public static void spawnTextParticle(ParticleOptions effect, double x, double y, double z, Component text, double scaling, String expression, double dpb, double vx, double vy, double vz, int age, String speedExpression, double speedStep, String group) {
        // 1.21.6+ 的离屏字体绘制必须在渲染阶段进行，所以这里排队、由 HUD 回调驱动（见 TextUtil.requestImage）
    ...
    private static class VideoConsumer implements Predicate<BufferedImage> {
        ...
        /** 诊断：`-Dexparticle.selftest.video=true` 时逐帧打印被写入的颜色，用来判定「视频是否真的在播」。 */
        private static final boolean VIDEO_TRACE = Boolean.getBoolean("exparticle.selftest.video");
        private int frame;
        ...
        public boolean test(BufferedImage image) {
            ...
            if (init == 0) {
                ...
                for (int row = 0; row < dh; ++row) {
                    for (int col = 0; col < dw; ++col) {
                        if (particles[row][col].isAlive()) {
                            ...
                            float green = (float) ((pixel & 0xff00) >>> 8) / 255.0F;
                            float blue = (float) (pixel & 0xff) / 255.0F;
                            ((IParticle) particles[row][col]).setColorRGB(red, green, blue);
                            alive = true;
                            aliveNow++;
                        }
                    }
                ...
                if (VIDEO_TRACE) {
                    // 打印「中心像素」这一帧写进去的颜色：视频在播 → 每帧颜色不同
                    int center = result.getRGB(dw / 2, dh / 2);
                    System.out.println("[exparticle] video frame " + frame + ": center=0x" + Integer.toHexString(center)
                        + " alive=" + aliveNow + "/" + (dw * dh));
                }
                aliveCount = aliveNow;
                frame++;
                return alive;
